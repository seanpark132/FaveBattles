...
export default function AddNewVideo({ setChoicesData }) {
  ...
  async function handleAddVideo() {
    ...
    const choiceId = v4();
    const res = await (
      await fetch(`https://noembed.com/embed?dataType=json&url=${actualUrl}`)
    ).json();
  ...
  return (
    <section className="mb-6 flex max-w-4xl flex-col px-6">
      ...
      <button
        type="button"
        className={`mt-2 rounded border-transparent p-2 text-lg ${
