	...
	async function deleteGame(gameId) {
		try {
			...
			if (props.gameType === "image") {
				await Promise.all(
					props.choices.map(async (choice) => {
						const imgRef = ref(
							storage,
		...
		<div className="home-box-container">
			<div className="h-48 w-full overflow-hidden flex">
				<img
					...
					src={
						props.gameType === "video-youtube"
							? props.choices[0].thumbnailUrl
							: props.choices[0].url
					}
					alt="left img"
				/>
			...
			<h3 className="my-1 mx-2 max-h-16 overflow-hidden">
				[{props.mainCategory}] {props.title} ({props.choices.length}{" "}
				choices)
			</h3>
